...
const Layout = () => <Outlet />;
const Loader = () => <div className="flex flex-col items-center justify-center gap-3 py-16" role="status" aria-live="polite">
...
const RouteError = ({
  error,
  resetErrorBoundary
}: FallbackProps) => <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
    ...
    <button type="button" onClick={resetErrorBoundary} className="rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-sm transition hover:bg-primary/90">
      Réessayer
    </button>
  </div>;
...
const withGuards = (Component: LazyRouteComponent, options?: GuardOptions) => {
  const element = <ErrorBoundary FallbackComponent={RouteError}>
  ...
  return <RequireAuth roles={options.roles}>
      {element}
    </RequireAuth>;
};
const RequireAuth = ({
...
export function AppRoutes() {
  return <Routes>
      <Route path="/" element={<Layout />}>
        ...
        {/* B - ESPACE CLIENT */}
        <Route path="dashboard-client" element={withGuards(DashboardClient, {
        requiresAuth: true
      })} />

        {/* C - ESPACE ADMIN */}
        <Route path="dashboard-admin" element={withGuards(DashboardAdmin, {
        requiresAuth: true,
        roles: ["admin"]
      })} />
