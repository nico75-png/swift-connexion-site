...
const DashboardClient = () => {
  ...
  const sortedSections = useMemo(() => {
    const sectionsWithNotifications = NOTIFICATION_SECTIONS.map((section) => {
      ...
      const highestPriority = notifications.reduce<number>(
        (max, notification) => {
  ...
  useEffect(() => {
    ...
    const firstNotificationButton =
      notificationsRef.current?.querySelector<HTMLButtonElement>(
    ...
    <div className="flex h-screen bg-slate-100 text-slate-900">
      ...
      <aside className="relative hidden w-72 shrink-0 bg-slate-950/95 text-slate-100 shadow-2xl lg:flex lg:flex-col">
              ...
              return (
                <div key={item.id} className="relative">
                  ...
                  <motion.button
                    ...
                    className={`group relative flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-medium transition-all duration-300 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/60 ${isActive ? "text-white" : "text-slate-300 hover:text-white focus-visible:text-white"}`}
                    whileHover={{
                      x: 4,
                    }}
                    whileTap={{
        ...
        <div className="relative border-t border-white/10 px-6 py-6 text-xs text-slate-300">
          <p className="font-medium uppercase tracking-[0.28em] text-blue-200/70">
            Assistance
          </p>
          <p className="mt-2 text-sm text-slate-200/90">
            Support disponible 7j/7 via le centre d'aide One connexion.
          </p>
        </div>
      </aside>
        ...
        <div className="border-b border-slate-200/80 bg-white/90 px-4 py-4 backdrop-blur lg:hidden">
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-0.5">
              <p className="text-[11px] uppercase tracking-[0.28em] text-slate-500/80">
                One connexion
              </p>
          ...
          <nav className="mt-4" aria-label="Navigation principale mobile">
              ...
              {SIDEBAR_ITEMS.map((item) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => handleSectionChange(item.id)}
                    className={cn(
                      "relative inline-flex shrink-0 items-center gap-2 overflow-hidden rounded-full px-4 py-2 text-sm font-medium transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40",
                      isActive
                        ? "text-white shadow-[0_8px_24px_-16px_rgba(15,23,42,0.45)]"
                  ...
                  >
                    {isActive ? (
                      <motion.span
                        layoutId="mobileSidebarItem"
                        className="absolute inset-0 rounded-full bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900"
                        transition={{
                          type: "spring",
                          stiffness: 380,
                          damping: 32,
            ...
            <div className="flex items-center gap-4">
              <div className="relative" ref={notificationsRef}>
                <motion.button
                  ...
                  className={cn(
                    "relative flex h-11 w-11 items-center justify-center rounded-xl border transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40",
                    isNotificationsOpen
                      ? "border-blue-200/70 bg-blue-50/90 text-blue-600 shadow-[0_10px_28px_-20px_rgba(37,99,235,0.8)]"
                      : "border-slate-200/70 bg-white/70 text-slate-500 hover:border-blue-200/70 hover:bg-blue-50/70 hover:text-blue-600",
                  )}
                  aria-label={
                    isNotificationsOpen
                  ...
                  <AnimatePresence>
                    {unreadCount > 0 ? (
                      <motion.span
                        ...
                        className="absolute -right-1 -top-1 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-gradient-to-br from-blue-500 via-indigo-500 to-blue-600 px-1 text-[10px] font-semibold text-white shadow-lg shadow-blue-500/30"
                        aria-hidden="true"
                      >
                        ...
                        <div className="flex items-start justify-between gap-4 border-b border-slate-200/60 bg-gradient-to-r from-slate-50 via-white to-slate-50 px-5 py-4">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-slate-500">
                              Priorisez vos actions en un coup d'œil
                            </p>
                          </div>
                          <button
                            type="button"
                          ...
                          <div className="space-y-6 px-5 py-5">
                            {sortedSections.length > 0 ? (
                              ...
                              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white/60 px-6 py-12 text-center text-sm text-slate-500">
                                <BellRing
                                  className="mb-3 h-6 w-6 text-slate-400"
                                  aria-hidden="true"
                                />
                                ...
                                <p className="mt-1 text-xs text-slate-400">
                                  Tout est à jour, revenez plus tard pour de
                                  nouvelles alertes.
                                </p>
                              </div>
                            )}
              ...
              <div className="flex items-center gap-3 border-l border-slate-200 pl-4">
                <div className="text-right">
                  ...
                  <p className="text-xs text-slate-500">
                    clara.dupont@one-connexion.com
                  </p>
                </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
              ...
              >
                {isSigningOut ? (
                  ...
                  <LogOut
                    className="h-4 w-4 text-gray-500 transition-colors duration-200 group-hover:text-red-600"
                    aria-hidden="true"
                  />
                )}
